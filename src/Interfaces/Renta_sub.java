...
public class Renta_sub extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Renta_sub frame = new Renta_sub(transpor);
	...
	**
	 * Create the frame.
	 */
	public Renta_sub(medioDeTransporte[] transpor) {
		setTitle("Submarino");
		setIconImage(Toolkit.getDefaultToolkit().getImage(Renta_sub.class.getResource("/icons/logo2.png")));
		...
		comboBox.setFont(new Font("Arial", Font.ITALIC, 15));
		comboBox.setName("");
		...
		for (int i = 0;i<transpor.length;i++) {
			if (transpor[i] instanceof Submarino) {
				comboBox.addItem("Submarino "+transpor[i].getTipo());
			}
		}
		...
		lblNewLabel_1.setIcon(new ImageIcon(Renta_sub.class.getResource("/icons/1491254414-departuredatecalendartravel_82969.png")));
		...
		lblFechaDeEntrega.setIcon(new ImageIcon(Renta_sub.class.getResource("/icons/1491254415-returnarrivaldatecalendar_82967.png")));
		...
		label_1.setBounds(408, 350, 46, 14);
